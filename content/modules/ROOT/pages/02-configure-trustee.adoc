...
Note that if you defined `$CONTAINER_IMAGE_SIGNATURE_TYPE` to enforce container image signature verification, then the `kbsSecretResources` field should also contain `$CONTAINER_IMAGE_SIGNATURE_TYPE`. In this workshop we assume it is not needed so it is not added.

Optionally only if you did the optional steps:
* For TDX PCCS you can add the `tdxConfigSpec` section
