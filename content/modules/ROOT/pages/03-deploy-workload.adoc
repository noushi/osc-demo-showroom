...
+
There are multiple ways to accomplish this, for example when xref:02-configure-osc.adoc#pp-key[setting up the ssh key in OSC]. If `id_rsa` is not destroyed, it could be used to try and ssh into the VM. However, such attempt will fail, as the VM does not accept any SSH incoming connection.
...
. It is also possible to inspect Trustee logs to understand how the process worked. Exit from the pod and run the following:
+
[source,sh,role=execute]
----
