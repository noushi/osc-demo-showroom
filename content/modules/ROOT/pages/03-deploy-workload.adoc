...
In this example you will learn the minimum addition in the podspec that CoCo brings in traditional pod deployment. No other action is necessary w.r.t the pod itself, and the confidential VM is completely transparent to it.

This is a sample yaml that runs an `hello-openshift` pod. This pod acts as server and outputs `"Hello Openshift!"` every time is reached. The difference between this pod deployed as confidential container and traditional pod is just that the former has `spec.runtimeClassName: kata-remote` defined in the pod spec.
...
----
+
IMPORTANT: notice how the `curl` call is asking for `http://127.0.0.1`. This is done purposefully, because the pod itself doesn't have to know where the Trustee lives, what is its ip and such information. This is provided in the OSC `AA_KBC_PARAMS` given in the xref:02-configure-osc.adoc#pp-cm[peer-pods configmap]. Such information is then forwarded to the local `Trustee agent` running in side the CoCo Confidential VM automatically, so all the pod has to do is communicate **locally** (therefore http is enough) with the local `Trustee agent` and forward it the path to the secret it would like to get, in this case `kbsres1/key1`. The `Trustee agent` will then take care of collecting hardware & software attestation proofs, create an attestation report, enstablish an `https` connection with the remote attester `Trustee operator`, and then perform attestation.
...
----
...
[2025-02-16T11:46:53Z INFO  api_server::http::attest] Attest API called.
[2025-02-16T11:46:53Z INFO  attestation_service] AzSnpVtpm Verifier/endorsement check passed.
[2025-02-16T11:46:53Z INFO  attestation_service] Policy check passed.
...
[2025-02-16T11:51:19Z INFO  api_server::http::resource] Get resource from kbs:///default/kbsres1/key1
[2025-02-16T11:51:19Z INFO  api_server::http::resource] Resource access request passes policy check.
[2025-02-16T11:51:19Z INFO  actix_web::middleware::logger] 10.131.0.9 "GET /kbs/v0/resource/default/kbsres1/key1 HTTP/1.1" 200 514 "-" "attestation-agent-kbs-client/0.1.0" 0.001004
----
+
