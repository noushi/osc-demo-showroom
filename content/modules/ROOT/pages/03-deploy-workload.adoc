...
----
  * Log into one of the CAA pods running in the worker nodes:
+
[source,sh,role=execute]
----
...
[root@aro-cluster-q5hqf-xs7zb-worker-eastus1-6rlsl /]# # <--- Notice how we are now in worker node!
----
...
----
[root@aro-cluster-q5hqf-xs7zb-worker-eastus1-6rlsl /]# # <--- Notice how we are now in worker node!
[root@aro-cluster-q5hqf-xs7zb-worker-eastus1-6rlsl /]# PEER_POD_VM_IP=10.0.2.10
[root@aro-cluster-q5hqf-xs7zb-worker-eastus1-6rlsl /]# ssh peerpod@$PEER_POD_VM_IP
[...]
